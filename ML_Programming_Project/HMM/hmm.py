...
class HMM:
        ...
        for t in range(1, L):
            for s in range(S):
                ...
                for s_dash in range(S):
                    a = a + self.A[s_dash][s] * alpha[s_dash][t - 1]
                alpha[s][t] = self.B[s][O[t]] * a
        return alpha
    ...
    def backward(self, Osequence):
        ...
        for tr in range(0, T):
            ...
            for s in range(S):
                a = 0
                for s_dash in range(S):
                    a = a + self.A[s][s_dash] * self.B[s_dash][O[t + 1]] * beta[s_dash][t + 1]
                beta[s][t] = a
        return beta
    ...
    def posterior_prob(self, Osequence):
        ...
        T = len(Osequence)
        gamma = np.zeros([S, T])
        normalizer = self.sequence_prob(Osequence)
        for i in range(T):
            for s in range(S):
                a = alpha[s][i] * beta[s][i]
                gamma[s][i] = a / normalizer
        return gamma
    ...
    def likelihood_prob(self, Osequence):
        ...
        for i in range(L - 1):
            for s in range(S):
                for s_dash in range(S):
                    prob[s, s_dash, i] = self.A[s, s_dash] * self.B[s_dash, O[i + 1]] * beta[s_dash, i + 1] * alpha[s, i] / normalizer
        return prob
        ...
        for t in range(1, L):
            for s in range(S):
                ...
                for s_dash in range(S):
                    ...
                    if delta_temp > maxdelta:
                        maxdelta = delta_temp
                        argmaxdelta = s_dash
                delta[s][t] = self.B[s][O[t]] * maxdelta
                big_delta[s][t] = argmaxdelta
        # Backtracking
        z_star = []
