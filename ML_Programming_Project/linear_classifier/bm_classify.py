...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    newX = np.insert(X, 0, 1, axis=1) # inserting 1 as first dimension to X
    newW = np.insert(w, 0, b, axis=0) # inserting b to W
    y = np.where(y == 0, -1, 1)
    ...
    if loss == "perceptron":
        ...
        ################################################
        for i in range(max_iterations + 1):
            y_wt_x = y * np.dot(newX, newW)
            sigma_of_y_wt_x = sigmoid(-y_wt_x)
            sigma_of_y_wt_x_y = sigma_of_y_wt_x * y
            sigma_of_y_wt_x_y_x = np.dot(sigma_of_y_wt_x_y, newX)
            eeta_by_N = step_size / N
            newW = newW + (eeta_by_N) * sigma_of_y_wt_x_y_x
